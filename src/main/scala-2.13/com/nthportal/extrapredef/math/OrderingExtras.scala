...
object OrderingExtras {
  ...
  object Implicits extends Ordering.ExtraImplicits {
    override implicit def infixOrderingOps[T](x: T)(implicit ord: Ordering[T]): Ordering[T]#OrderingOps with ExtraOps[T] = {
      new ord.OrderingOps(x) with ExtraOps[T]
    }
  }
