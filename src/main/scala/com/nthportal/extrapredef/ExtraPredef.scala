...
trait ExtraPredef extends VersionSpecificExtraPredef {
   ...
   * [[Predef.require(requirement:Boolean):Unit* `Predef.require`]], but tests a state instead of a method argument.
   *
   * @param requirement
  ...
  /**
   * Tests an expression, throwing an [[IllegalStateException]] if false. This method is similar to
   * [[Predef.require(requirement:Boolean,message:=>Any):Unit* `Predef.require`]], but tests a state instead of a method
   * argument.
   *
   * @param requirement
   *   the expression to test
...
object ExtraPredef extends ExtraPredef {
  ...
  final class ExtraRichOrderedInt(private val prev: Int) extends AnyVal {
    ...
    /**
     * Returns the result of the immediately previous comparison if it was not `0`; otherwise the result of comparing
     * two other things (`a1` and `a2`).
     *
     ...
     * }}}
     *
     * While this method can called on arbitrary [[Int]] s, to be meaningful, it should only be called on the result of
     ...
     * @param ord
     *   an [[Ordering]] for `a1` and `a2`
     * @tparam A
     *   the type of `a1` and `a2`
     * @return
     *   the result of the previous comparison if it was not `0`, or the result of comparing the two other things
     *   otherwise
     */
  ...
  final class ExtraRichOrdered[A <: Ordered[A]](private val self: A) extends AnyVal {
    ...
    /**
     * Returns `true` if `this` and `that` are unequal by their [[java.lang.Comparable natural ordering]]; `false`
     * otherwise. (That is, returns `true` if `this` is greater than or less than `that`.)
     *
     * @param that
    ...
    /**
     * Returns `true` if `this` and `that` are equal by their [[java.lang.Comparable natural ordering]]; `false`
     * otherwise. (That is, returns `true` if `this` is neither greater than nor less than `that`.)
     *
     * @param that
     *   the thing to which to compare `this`
     * @return
  ...
  final class ExtraRichOption[+A](private val self: Option[A]) extends AnyVal {
     ...
     *
     * The Try returned is a [[scala.util.Success Success]] with the value of this Option if this Option is defined, or
     * a [[scala.util.Failure Failure]] with a `NoSuchElementException` if this Option is empty.
     *
     * @return
     *   a Try from this Option
     */
     ...
     *
     * The Future returned succeeds with the value of this Option if this Option is defined, or fails with a
     * `NoSuchElementException` if this Option is empty.
     *
     * @return
    ...
    /**
     * Returns an [[Option]] by applying `ifDefined` to the value of this Option if this Option is defined, or `ifEmpty`
     ...
     * @return
     *   an Option by applying `ifDefined` to the value of this Option if this Option is defined, or `ifEmpty` if this
     *   Option is empty
     */
    ...
    /**
     * Returns an [[Option]] containing the specified value if this Option is empty, or an empty Option if this Option
     * is defined.
     *
     * @param ifEmpty
     ...
     * @return
     *   an Option containing the specified the specified value if this Option is empty, or an empty Option if this
     *   Option is defined
     */
  ...
  final class ExtraRichTry[+A](private val self: Try[A]) extends AnyVal {
     ...
     *
     * The Future returned succeeds with the value of this Try if it is a [[scala.util.Success Success]], or fails with
     * a `NoSuchElementException` if this Try is a [[scala.util.Failure Failure]].
     *
     * @return
  ...
  final class ExtraRichEither[+L, +R](private val either: Either[L, R]) extends AnyVal {
     ...
     *
     * The Future returned succeeds with the `Right` value if this Either is a [[scala.util.Right Right]], or fails with
     * the `Left` value if this Either is a [[scala.util.Left Left]].
     *
     * @return
  ...
  final class ExtraRichSortedMap[K, +V](private val map: collection.SortedMap[K, V]) extends AnyVal {
    ...
    /**
     * Returns `true` if this [[collection.SortedMap `SortedMap`]] contains the same mappings in the same order as
     * another `SortedMap`; `false` otherwise.
     *
     * @param other
     *   the other `SortedMap`
     * @return
     *   `true` if this `SortedMap` contains the same mappings in the same order as `other`; `false` otherwise
     */
